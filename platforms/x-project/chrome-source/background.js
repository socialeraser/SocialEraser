// Eraser for X Background Script (Service Worker)
...
async function injectContentScriptToMatchingTabs() {
  try {
    ...
    if (!tabs || tabs.length === 0) {
      console.log('[Eraser for X] No matching tabs to inject into');
      return;
    }
    console.log('[Eraser for X] Injecting content script into', tabs.length, 'matching tab(s)');
    ...
    for (const tab of tabs) {
      try {
        ...
        console.log('[Eraser for X] Skip inject tab', tab.id, ':', e.message);
      }
    }
  } catch (e) {
    console.warn('[Eraser for X] Failed to query matching tabs:', e);
  }
}

// 插件安装 / 更新触发：拉配置 + 注入已有 tab
chrome.runtime.onInstalled.addListener(() => {
  console.log('[Eraser for X] Extension installed');
...
chrome.runtime.onStartup.addListener(() => {
  console.log('[Eraser for X] Chrome started');
  loadConfigInBackground();
  injectContentScriptToMatchingTabs();
});
...
async function saveConfigToStorage(config) {
  try {
    ...
    console.log('[Eraser for X] Config saved to storage');
  } catch (e) {
    console.warn('[Eraser for X] Failed to save config to storage:', e);
  }
}
...
async function loadConfigFromStorage() {
  try {
    ...
    console.warn('[Eraser for X] Failed to load config from storage:', e);
  }
  return null;
}
...
async function loadConfigInBackground(forceReload) {
    ...
    try {
      console.log('[Eraser for X] Background: Fetching config from:', CONFIG_URL);
      ...
      if (response.ok) {
        ...
        console.log('[Eraser for X] Background: Config loaded successfully');
        return config;
      } else {
        console.warn('[Eraser for X] Background: Config fetch failed, HTTP', response.status);
      }
    } catch (error) {
      // 网络异常 / CORS / 超时等都到这里
      console.warn('[Eraser for X] Background: Config fetch failed:', error.message);
    }

    // 远程 fetch 失败 → 从 storage 拿上次缓存的（兜底）
    const stored = await loadConfigFromStorage();
    if (stored) {
      console.log('[Eraser for X] Background: Using stored config as fallback');
      return stored;
    }
    ...
    try {
      ...
      console.log('[Eraser for X] Background: Loading bundled default config from:', defaultUrl);
      ...
      if (defaultResponse.ok) {
        ...
        console.log('[Eraser for X] Background: Bundled default config loaded as fallback');
        return defaultConfig;
      } else {
        console.warn('[Eraser for X] Background: Bundled default config fetch failed, HTTP', defaultResponse.status);
      }
    } catch (e) {
      console.warn('[Eraser for X] Background: Failed to load bundled default config:', e.message);
    }
      ...
      if (tab) {
        ...
        chrome.tabs.sendMessage(tab.id, forwardedMessage).then(sendResponse).catch(function(e) {
          ...
          if (e && e.message && e.message.indexOf('message channel closed') >= 0) {
            console.log('[Eraser for X] Content page navigated, message channel closed (expected)');
          } else {
            console.error('[Eraser for X] Failed to send to content:', e);
          }
    ...
    getXTab().then(function(tab) {
      if (tab) {
        chrome.tabs.update(tab.id, { url: message.url }).catch(function(e) {
          console.error('[Eraser for X] Failed to navigate tab:', e);
        });
      } else {
        console.warn('[Eraser for X] forceNavigation: no X tab found');
      }
    });
    sendResponse({ success: true });
    return true;
...
function handleBackgroundMessage(message, sendResponse) {
  switch (message.type) {
    case 'getStatus':
      ...
      break;
    case 'log':
      console.log('[Eraser for X]', message.data);
      sendResponse({ success: true });
      break;
    default:
...
chrome.sidePanel.setPanelBehavior({ openPanelOnActionClick: true }).catch(function(err) {
  console.error('[Eraser for X] Failed to set panel behavior:', err);
});
