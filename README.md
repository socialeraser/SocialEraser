...
- dailyUsage 计数竞态 → 单飞链 `_dailyUsageChain`（**待补 verify-daily-usage-chain**）
...
node scripts/verify-sidepanel-bindings.js  # 6 项：sidepanel.js 所有 els.xxx 引用必须有 getElementById 绑定（防加新元素忘绑）
...
node scripts/verify-tweets-sub-options-grouping.js  # 7 项：Tweets 子项展开时联动分割线（防"Tweets+子项"被 border-bottom 切到 Likes 那组）
```
7. 勾选要执行的操作，点击「开始清理」
